...
export function AllPlotsClient({
  plots,
  experiences,
}: {
  plots: Plot[];
  ...
  useEffect(() => {
    if (locationParam && locationParam !== ALL_LOCATIONS) {
      setFilteredPlots(plots.filter((p) => p.location === locationParam));
      setFilteredExperiences(
        experiences.filter((e) => e.location === locationParam)
      );
    } else {
      setFilteredPlots(plots);
      setFilteredExperiences(experiences);
    }
  }, [locationParam, plots, experiences]);
        ...
        >
          <div className="flex justify-center mb-8 border-b border-gray-200">
            <TabsList className="bg-transparent p-0 h-auto gap-8 sm:gap-12 justify-center w-full text-gray-500">
              ...
              >
                <div className="absolute top-1 -right-3 bg-slate-700 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md shadow-sm border border-slate-600 tracking-wider">
                  NEW
                </div>

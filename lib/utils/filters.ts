...
export function filterPlots(plots: Plot[], filters: FilterOptions): Plot[] {
  return plots.filter(plot => {
    ...
    const area = convertAreaToNumber(plot.area);
    const matchesPrice = price >= filters.priceRange[0] && price <= filters.priceRange[1];
    const matchesArea = area >= filters.areaRange[0] && area <= filters.areaRange[1];
    const matchesLocation = filters.location === ALL_LOCATIONS || plot.location.toLowerCase() === filters.location.toLowerCase();
    const matchesSearch = !filters.searchQuery || 
      ...
      plot.description.toLowerCase().includes(filters.searchQuery.toLowerCase());
